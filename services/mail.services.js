...
function enviarCorreoVerificacion(usuario, correoDestino, token) {
  ...
  const msg = {
    ...
    html: `<p>Haz clic en el siguiente enlace para verificar el registro del usuario <strong>${usuario.name} ${usuario.surname}</strong><a href="${usuario.email}"> (${usuario.email}) </a>a tu empresa: <a href="${link}">${link}</a></p>`,
  };
