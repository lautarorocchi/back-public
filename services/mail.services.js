...
function enviarCorreoVerificacion(usuario, correoDestino, token) {
  ...
  const msg = {
    ...
    html: `<p>Haz clic en el siguiente enlace para verificar el registro del usuario <strong>${usuario.name} ${usuario.surname} (Email:${usuario.email}) a tu empresa: <a href="${link}">${link}</a></p>`,
  };
